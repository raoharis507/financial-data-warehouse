...
from pyspark.sql.functions import avg, col, stddev, lag
...
class SparkAnalyticsService:
    ...
    async def calculate_moving_average_spark(cls, asset_id: str, provider: str, days: int = 5):
        ...
        result = spark.sql(f"""
            SELECT assetId, 
                   AVG(closePrice) OVER (ORDER BY timestamp DESC ROWS BETWEEN {days-1} PRECEDING AND CURRENT ROW) as moving_average
            FROM prices
            LIMIT 1
        """).collect()
        
        ma_value = float(result[0][1]) if result else 0
